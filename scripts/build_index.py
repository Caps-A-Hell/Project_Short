...
soglia = oggi - datetime.timedelta(days=GIORNI_CONSERVAZIONE)

rimossi = 0
for f in list(Path(".").glob("*.html")) + list(Path(".").glob("*.json")):
    m = pattern.match(f.name)
    if m:
        data_file = datetime.date.fromisoformat(m.group(2))
        if data_file < soglia:
            f.unlink()
            rimossi += 1

if rimossi:
    print(f"Rimossi {rimossi} file più vecchi di {GIORNI_CONSERVAZIONE} giorni (prima del {soglia.isoformat()})")

# --- Raccolta di tutti i file lingua-data presenti (dopo la pulizia) ---
trovati = {}
